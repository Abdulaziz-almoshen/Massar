...
- 2026-08-16 U93 [stated]: R93 — CORRECTION ON HOW I ASK, plus a decided product rule. His words: 'You have to be professional. Why do you ask me the slim question?' I had handed back an obvious product decision (does 'don't contact me again' mean marketing-stop or hard opt-out) as if it were his to make. It was not - it is a decision a competent product owner makes and REPORTS. THE RULE HE GAVE: 'we're not contacting anymo…
- 2026-08-16 U92 [stated]: R92 — MAJOR RE-FRAME, supersedes the R60/R75 closer spec for what the agent's JOB is. His words: 'this is not sales skills. Please redo your work.' The agent is a SORTER, not a closer. Required behaviour: (a) prospect asks a question -> answer their most important question, then connect them to a salesperson; (b) interested -> establish a SCHEDULED TIME ('when are we going to schedule them?'); (c)…
- 2026-08-16 U86 [stated]: R86 + R88: two durable items. (1) STANDING TEST CONTRACT, his words: 'test the agent inputs and outputs with codex. make sure Claude is not the one communicating with customers we are using openai key for this project. make sure codex model test our agent and fine tune the agent prompt till it is 100% ready for prod. must be market competitive.' So: the agent runs on HIS OpenAI key (gpt-5.6-terra)…
- 2026-08-16 U83 [stated]: R83 + R84/R85: three signals from one exchange. (1) BUG REPORT: the contact profile hung forever on «جارٍ تجميع ملف العميل» - measured as a 401 with no error state, not slow loading; every endpoint returned 200 in under 0.75s. (2) THE REAL COMPLAINT, and it is PROVENANCE not analytics: 'I see all campaign details and not sure which one is related to the last one to measure campaign performance.' D…
- 2026-08-16 U79 [stated]: R79, founder review pass 3 - the sharpest articulation of the acceptance bar so far. CORE BEHAVIOURAL RULE, verbatim: 'Never sound like you are executing a sales process. Sound like you are having a commercial conversation. The CRM/process should happen internally.' Seven specifics: (1) NEVER assume deal status, blockers, buying intent, readiness or commitment - the line about price being the only…
- 2026-08-16 U75 [stated]: R75, SECOND authoritative AE spec, supersedes R60 - full rewrite of the agent's sales prompt plus one new hard rule. Structure adopted verbatim: role, sales motion, account-context-as-memory, PRODUCT LOCK (critical), 9 sales states inferred not forced, pre-response checklist, ANSWER-VALUE-NEXT STEP golden rule, discovery-only-if-it-changes-a-decision, answer-direct-questions-first, outcomes-before…
- 2026-08-16 U72 [stated]: R72, founder's structured review of a live NVR transcript - the agent behaves like a qualification workflow, not a salesperson. THREE HARD RULES, now enforced: (1) PRODUCT LOCK - once a product is established every attachment, price, feature and question stays scoped to it until the CUSTOMER explicitly switches; sending Sick Leave material into an NVR conversation is the defect that must never rec…
- 2026-08-14 U65 [stated]: R65, STANDING CONSTRAINT: 'Never use Claude for any agentic responses. Make OpenAI respond with the messages.' Every customer-facing generation - agent replies, campaign openers, any future agentic surface - must run on OpenAI. Claude is the engineering session only and must never enter the runtime message path. Verified at the time of the instruction: zero anthropic/claude references in massar-en…
- 2026-08-14 U60 [stated]: R60: he supplied the AUTHORITATIVE sales spec for the agent - senior B2B AE running usage-led expansion on existing healthcare accounts. This is a source-of-truth document, not a suggestion: it defines identity, the current-state to future-state motion (workflow into HIS, never sell the API), 13 strategies, 10 rules, banned phrasings, and message design (1 idea + 1 next step, 2-5 short sentences, …
