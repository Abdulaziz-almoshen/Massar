...
TASK_CTX = (
    ...
    "reason logged. "
    "**STRICT CAPABILITY CONTRACT:** on every substantial task, actual named owners MUST complete "
    "Product Discovery → Business Analysis → Market/Prior-Art Research → Planning before build, then "
    "Safety → Reviewer → QA Engineer → deterministic delivery-quality gate → CPO acceptance → Reporter. "
    ...
    "Designer before implementation. A role mentioned in prose, treated as a private 'lens', or listed "
...
def main() -> None:
    ...
    # System/background turns are NOT user requests: stamping a route boundary for them
    # orphans the current run's completed stage events and makes the Stop hook falsely
    # report skipped stages (observed 3x on Aug 10-11, 2026 — see STATE.md decision log).
    _SYSTEM_TURN_MARKERS = (
        "[SYSTEM NOTIFICATION - NOT USER INPUT]",
        "<task-notification>",
        "Stop hook feedback:",
        "[orbit] STRICT CAPABILITY CONTRACT",
        "[orbit] USER MEMORY GATE",
        "[orbit] DELIVERY QUALITY GATE",
        "[orbit] OBSERVABILITY",
    )
    if any(m in prompt for m in _SYSTEM_TURN_MARKERS):
        sys.exit(0)                          # no classify, no stamp, no injection
