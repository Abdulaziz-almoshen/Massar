...
و٢٢٥ اختبار وحدة و٣٥ اختبارًا على قاعدة بيانات حقيقية.

**أ — العناصر البصرية المشتركة.** نقطة الحالة كانت مُعرَّفة خمس مرات بثلاثة أحجام، اثنتان منها
بألوان مكتوبة داخل السطر لا يراها تغيير الرموز. `crm-primitives.ts` يعرّفها مرة واحدة، قبل كتابة
الشاشات الأربع الجديدة لا بعدها.

**ب — الباقات والسعر المنشور.** الصوت الخارجي وجد قائمة الأسعار الحقيقية في `agent.ts`: منتج واحد
من ستة له سعر منشور، والخمسة الباقية تقول «يحدده المختص». لذلك `offListPct` يعيد `null` لا صفرًا.
مفتاح خارجي مركّب يمنع ربط صفقة بباقة منتج آخر — مُختبَر على قاعدة حية.

**ج — القطاعات.** ثلاثة قطاعات سوقية (مستشفيات · صيدليات · أعمال) تُزرع عند الإقلاع لا داخل
هجرة، لأن الهجرة ليست كاتبًا: تعمل مرة واحدة على قاعدة واحدة. الزرع «أدرج إن غاب» فلا يمحو تصحيحًا
يدويًا عند النشر التالي — مُثبَت بإعادة الإقلاع بعد تصحيح.
`bestFor` في `agent.ts` **نقض التصنيف الذي استنتجتُه**: «الإجازات المرضية»، المنتج الوحيد بسعر
منشور، منتج مستشفيات لا أعمال. منتجان مُعلَّمان «مُستنتَج» حيث يناقض `bestFor` التصنيف.
والإنتاج يحمل منتجَين لا يعرفهما `agent.ts` إطلاقًا («سجل التطعيمات الوطني»، «صحة أعمال Plus») —
يظهران في دلو «بلا قطاع» صراحةً بأرقامهما، لا يسقطان من المجاميع.

**د — ستة أبواب بدل خمسة عشر.** كل ما غادر الشريط احتفظ بوجهة كتبويب تحت بابه، وأي `#hash` قديم
ما زال يعمل لأن أيًّا من معالجات المسارات لم يُمَسّ. خطوة بوابة جديدة (`check:nav`) تمنع اختفاء
شاشة: الدخان يتنقّل **بالـhash**، فهو يثبت أن الشاشة تُرسم ولا يثبت أبدًا أن إنسانًا يصل إليها.
`#home` خرج من نبضة الخمس ثوانٍ، وشارة «مباشر» معه — شارة حيّة فوق لقطة ثابتة أسوأ من لا شارة.

**هـ — التكامل المستمر.** لم يكن هناك CI إطلاقًا. الآن البوابة والاختبارات وشمّ ١١ مسارًا في
متصفّح حقيقي على كل دفعة. **اكتشافان من التشغيل لا من الكتابة**: `GUPSHUP_API_KEY` يجب أن يُضبط
فارغًا لا أن يغيب — `dotenv` لا يتجاوز متغيّرًا مضبوطًا، وبغيابه فقط أبلغ المحرك `outbound.ok:
true` أي أنه **قادر على الإرسال**؛ وسير العمل كما كُتب أولًا كان ليبقى أحمر للأبد لأن الدخان كان
يثبّت رقم هاتف بعينه.

**ما زال مفتوحًا**: شاشتا «المنتجات» و«التقارير» بابان بلا محتوى بعد؛ وقطاع المنتجَين الإنتاجيين
قرار للمؤسِّس؛ ورمز مندوب واحد يحتاج تدويرًا (`TODOS.md`).

---

## 2026-09-04 · [T4] سلّم واحد، وسجلّ لما يفعله المندوب · DEPLOYED

**كيف وُجدت**: أربع مراجعات متتابعة على ما شُحن فعلًا — CEO ثم هندسية، ولكل منهما صوت خارجي
(Codex). المراجعة الهندسية اتخذت ستّ قرارات معمارية، ثم **نقض الصوت الخارجي اثنين منها بالدليل**،
وهذا هو المكسب الحقيقي من تشغيله.

**العشرون مهمّة، تسعة عشر منها منجزة**:

1. **سلّم واحد بدل اثنين.** كانت دلالات المراحل موزّعة على نحو ١٥ موضعًا في سلّمين متوازيين، وهذا
   الازدواج هو سبب عيبَي المراحل معًا: مرحلتان (`discover` و`quote`) موجودتان في سلّم الأوزان وغير
   موجودتين في سلّم اللوحة، فكان `validateOppLine` يردّهما بـ400 رغم أن قيد Postgres يقبل الثماني
   منذ الهجرة ٠٠٢. و`CONFIRMED_INTEREST_STAGES` كان زوجًا مكتوبًا بخط اليد، فصفقة تنتقل من
   `tech` إلى `quote` تختفي من تغذية «مكين» ثم تعود عند التفاوض. `SALES_STAGES` هو المصدر الآن.

   **والفخّ الذي كدت أقع فيه**: اقترحتُ اشتقاق «الاهتمام المؤكّد» بـ`weightPct >= 65`، و«إغلاق – ربح»
   وزنه ١٠٠. التعليق في `opps-domain.ts:63` يحذّر من هذا حرفيًّا — أن اختبار المدى يطوي العملاء
   داخل رقم يقرؤه جانب المنتج كخطّ أنابيب. أمسكه الصوت الخارجي. الاستثناء الآن صريح ومؤكَّد باختبار.

2. **`engagements` — ما فعله المندوب، لا ما فعلته الصفقة.** `updateOpp` يكتب في السجلّ عند تغيّر
   المرحلة فقط، فمكالمة ينتهي فيها العميل بـ«اتصل بي الأسبوع القادم» لم تكن تُسجَّل شيئًا — وهي
   أغلب المكالمات. والبوّابة أ تَعُدّ التفاعلات. الجدول مُفتَّح على **جهة الاتصال** لا على الفرصة،
   لأن المندوب يجري مكالمة واحدة لإنسان واحد بينما الحساب يحمل خطّ منتجات لكلّ خدمة.

3. **أمر واحد، مفتاح واحد، معاملة واحدة.** تحديث الفرصة وحدث المرحلة والتفاعل ومهمّة الإدارة كلّها
   تحت `idem_key` واحد. إعادة الإرسال خمس مرّات تُنتج صفًّا واحدًا ومهمّة واحدة.

4. **النتيجة ليست أمرًا.** `resolveOutcome` يشتقّ المرحلة الهدف من نوع النتيجة مقابل المرحلة
   المقفلة بـ`FOR UPDATE`، فلم يعد ممكنًا إرسال `contact/interested` كانتقال إلى «إغلاق – خسارة».

5. **بيانات اعتماد ثالثة، ولكلّ مندوب.** `REP_TOKENS` أزواج «اسم:سرّ»، والفاعل يُشتقّ من بيانات
   الاعتماد لا من جسم الطلب — كان `adminName` يقرأ `req.body.by`، فأيّ متصل يوقّع سجلًّا باسم أيّ
   أحد، وهذا يُفقد أرقام البوّابة أ معناها. سطح `/rep` ثلاثة مسارات، مغلق افتراضيًّا (٤٠٤).

6. **«أين تتعثّر الصفقات» شُحنت مع كاتبها**، ومعها قاعدة إغلاق: شاشة بلا طريقة لإغلاق صفّ تمتلئ
   بإدارات قديمة ويتوقّف الناس عن قراءتها.

7. **البوّابة أ صارت قابلة للحساب.** أسبوع العمل بالرياض من الأحد إلى الخميس، **وأيام بلا نشاط
   تُحتسب أصفارًا** (الوسيط على الأيام التي فيها صفوف فقط يجيب سؤالًا آخر)، والمقام مُعلن،
   والتأريخ الرجعي محدود بيومين فلا يُصنَع امتثال من أسبوع هادئ.

8. **النسخة المقيمة من المحادثة لها مستهلكون متعارضون.** اقترحتُ نافذة ٩٠ يومًا؛ أثبت الصوت
   الخارجي أن `readSeriousness` **مقصود أن يكون عمريًّا** (`index.ts:734`)، فكان اقتراحي الخطأ
   نفسه بوحدة أخرى. فُصل المستهلكون: التجميعات الدقيقة من SQL، والقراءة العمرية عند فتح السجل.

9. **«ما يستحق اتصالك اليوم» صفحة منفصلة** لا تبويبًا في اللوحة: `/dashboard` يضع رمز المشرف في
   `localStorage`، وشاشة مندوب على النطاق نفسه ترث مدى انفجاره. **ونقرة لا تُبلّغ بنجاح لا تملكه** —
   ما لم يُرسَل يُحفظ بمفتاحه الأصلي ويُعاد إرساله، فلا يُحتسب مرّتين أبدًا.

**ما لم يُنجَز**: التوفيق بين الفروع. الإنتاج ينشر من `feat/product-interest-feed` وهو ٢٤ إيداعًا
أمام `master`، والدفع إلى مستودع عام نشرٌ يحتاج قرارك.

**الحُرّاس الجديدة**: `check-authz.mjs` (مصفوفة الصلاحيات عبر HTTP حقيقي، خطوة ١٩)، وأول مجموعة
اختبارات تلمس Postgres في تاريخ المشروع. كان لدينا ١٦١ اختبارًا ولا واحد منها يلمس قاعدة البيانات،
وهذا بالضبط كيف شُحن السجلّ بلا كاتب: **الجدول الفارغ نتيجة فارغة صحيحة**.

## 2026-09-04 · [T3] المحرّك التجاري — أربع مراجعات، والعيب الذي جمّد «المحقق»

**ما شُحن**: `src/sales-domain.ts` (ثماني مراحل موزونة، مستردّة من أرشيف الشركة نفسه)، شاشة
«المستهدفات والأداء» (`src/sales-crm.ts`)، هجرات مرقّمة مع قفل استشاري، وسبعة جداول جديدة.
ثم أربع مراجعات مستقلّة: أمنية، تقنية، تصميم، وCEO.

**العيب الأخطر، ووجدته مراجعة الـCEO لا أنا ولا المراجعة الأمنية ولا أيٌّ من ١٨ خطوة في البوّابة**:
`track_stage_events` لم يكن له كاتب. الإدراج الوحيد كان تعبئة الهجرة لمرّة واحدة، و`updateOpp`
يكتب `opportunities.stage` فقط. فالنتيجة أن صفقة يُعلّمها المندوب «ربح» تخرج من خطّ الفرص المفتوحة
ولا تصل «المحقق» أبدًا. قيس على نسخة حيّة قبل الإصلاح: صفقة بـ١٬٤٤٠٬٠٠٠ ر.س — المرحلة صارت `won`،
الأحداث المكتوبة صفر، «المحقق» ثابت عند ٣٬١٠٠٬٠٠٠، والعدد المفتوح ٣ ← ٢. المال غادر ولم يصل.
**«المحقق» كان مجمّدًا على قيم يوم الهجرة إلى الأبد، والشاشة كلّها زينة.**

الكاتب الآن داخل معاملة `updateOpp` نفسها مع `FOR UPDATE`، لا بجانبها. وإصلاحه كشف عيبًا ثانيًا في
استعلامي: صفقة تُربح ثم تُصحَّح للخلف كانت تبقى في «المحقق» **و** تعود إلى الفرص المفتوحة — المال
نفسه محسوبًا مرّتين في التغطية. السجلّ يقول **متى** حدث، و`opportunities.stage` يقول هل **ما زال**
صحيحًا، والاستعلام صار يستعمل الاثنين.

**الحارس**: `scripts/check-ledger-writers.mjs` — كل جدول تقرؤه شاشة يجب أن يكون له كاتب خارج
الهجرات. هذا الصنف من العيوب كلّف المشروع مرّتين الآن (`outbound.ts` تجاوزه ١٣ من ١٥ مُنادٍ لثلاثة
أسابيع بينما الحارس يطبعه في كل بناء)، فصار مؤكَّدًا في البوّابة لا متروكًا للذاكرة.

**الأمن — أول مراجعة أمنية مخصّصة في تاريخ المشروع، عشرة اكتشافات**. الأخطر: XSS مخزَّن يسرق
`ADMIN_TOKEN`. `JSON.stringify(x).replace(/"/g,"&quot;")` لا يهرّب `&`، فاسم منتج يحمل نصّ
`&quot;` حرفيًّا يفكّه مُحلّل HTML إلى اقتباس حقيقي ويخرج من المعالج. وأسماء المنتجات وسوم تُصنع من
خلية جدول مرفوع بفحص طول فقط. أُثبت بالهجوم ثم دُفع: الوسم
`&quot;-(window.__PWNED__=true)-&quot;` نفّذ على البناء المعيب ولم ينفّذ بعد الإصلاح.
والثاني: `scripts/seed-local.mjs` كان يستطيع إرسال واتساب — أمانه كان قائمة منع لأربعة مسارات،
والويبهوك مسار **خامس** إلى الأسلاك، وأحد الأسطر المبذورة «إيقاف» الذي يردّ عليه المعالج بإرسال.
الآن يقرأ `/health` ويرفض ما لم يكن `outbound.ok === false`.

**قرارات على المؤسِّس وحده، وكلّها حاملة**: الشهر الأول للسنة المالية؛ وهل المستهدف **حجوزات** أم
**ACV** أم **TCV** (الكود شحن TCV مسجَّلًا كاملًا في ربع الإغلاق، والخطة سمّت هذا سؤالها المفتوح
الوحيد — والشاشة الآن **تصرّح بأساسها** بدل أن تترك الرقم يوحي بأنه إيراد)؛ ولماذا مات النظام
السابق؛ والهيكل التنظيمي (`sectors` و`product_meta` بلا كاتب حتى يصل).

**عيبان معروفان لم يُصلحا بعد**: مرحلتان من الثماني — `discover` و`quote` — لا يمكن لأي واجهة
ضبطهما، لأن `opps-domain.ts` ما زال يحمل ستّ مراحل و`validateOppLine` يرفض ما عداها. فالتنبّؤ
الموزون يقفز ١٠٪ ← ٤٥٪ و٦٥٪ ← ٩٠٪. وترتيب البناء مخالف لقرار معتمد قبل يوم واحد: «المستهدفات
والأداء» **هي** تقرير الرئيس التنفيذي، وقد شُحنت قبل الشريحة أ التي كانت بوّابتها.

## 2026-09-04 · [T2] خمسة عيوب حيّة في مسار الإرسال والاستقبال · DEPLOYED

**كيف وُجدت**: لا بالبحث عن أخطاء، بل بالتحقق من ادّعاءات خطة المنصّة أثناء مراجعة `/autoplan`.
الادّعاء المركزي في الخطة كان خاطئًا: قالت إنه «لا توجد بوّابة إرسال في ١٥٬٧٨٠ سطرًا»، و`src/outbound.ts`
هي تلك البوّابة بالضبط — تُنفّذ الإيقاف والنافذة والرقم غير الصالح، وتُنادى من موضعين فقط من أصل ١٥.
والأهمّ أن `scripts/check-optout.mjs:92` كان يطبع هذا في كل بناء منذ ثلاثة أسابيع:
«NOTE: covers the 2 admin routes + guard logic. ~13 other call sites reach Gupshup».
حارسٌ كان يعلن العيب ولم يقرأه أحد.

**الخمسة، وكلها كانت تعمل في الإنتاج**:
1. **الويبهوك يقبل أي طلب حين لا يكون `WEBHOOK_TOKEN` مضبوطًا.** الشرط كان
   `if (cfg.webhookToken && ...)` فيصير خطأً حين يغيب السرّ، بينما `configReport()` يطبع
   «webhook is unauthenticated!» ويُقلع التطبيق. الآن يرفض حين يغيب. تُحقّق أولًا أن السرّ مضبوط
   في الإنتاج قبل الشحن.
2. **الويبهوك يُقرّ قبل التحليل**، ورمية واحدة تُسقط كل الأحداث بعدها، بعد الإقرار، حيث لا يظهر شيء.
   الآن: تحليل أولًا، وعزل لكل حدث، وتسجيل الجسم الخام — وأيضًا في الحالة الأهدأ: صفر أحداث من جسم
   غير فارغ، وهي التي كانت تبتلع «إيقاف» داخل مغلّف غير معروف. يبقى الإقرار ٢٠٠ عمدًا: الفشل حتميّ،
   ونقطةُ نهاية تردّ ٤xx في حلقة هي نقطة قد يُعطّلها المزوّد فنفقد الوارد كلّه. الديمومة عبر موت
   العملية تحتاج جدول `inbound_events` في المرحلة صفر، ومكتوبة كحدّ معروف لا كإيحاء.
3. **مهلة من المزوّد كانت تُسلّم الرسالة نفسها مرّتين.** لا `AbortSignal` في `postForm` (ولا في
   الملف كلّه)، والمساعد يُعيد الإرسال نصًّا عند أي رمية. الآن مهلة ١٥ ثانية، وتصنيف ثلاثيّ واحد
   مشترك: ٤xx أو ٢xx مع `status:error` = رفضٌ مؤكّد ← ارجع إلى النص؛ ٥xx أو مهلة = قد وصلت ← لا تُعِد
   أبدًا؛ رمية محليّة قبل الشبكة = لم تصل الأسلاك ← تبقى قابلة للإعادة. ومسار الحملة كان يحمل نسخته
   الخاصة من هذا الشرط بـ٤xx فقط، فصار يستدعي المشترك.
4. **`/health` كان يردّ `ok:true` دائمًا**، فآلةٌ فشلت هجرتها وسقطت إلى وضع الذاكرة فقط تُبلّغ أنها
   سليمة بينما كل كتابة تُهمل. صار صادقًا، ويبقى ٢٠٠ عمدًا: Fly يعيد التشغيل عند فشل الفحص، وحلقة
   إعادة تشغيل أثناء انقطاع قاعدة البيانات أسوأ من جسم يقرؤه المشغّل.
5. **إخفاقات الإرسال لكل مستلم كانت تُحسب وتُعاد ثم تُهمل في الواجهة**، فلا يرى المشغّل إلا
   «أُرسلت ٤٧ من ٥٠». الآن مجمّعة بالسبب ومعها أوائل الأرقام.

**ما أضافته `/review`**: خمس ملاحظات على النسخة الأولى من هذا الفرق، ثلاث منها لي، وواحدة انحدارٌ
أدخلتُه بنفسي — وسم «الدرجة الأولى» كمُسلَّمة بعد فشل محليّ لم يصل الشبكة كان سيمنع رسالةً لم تُرسل
قط. `tests/send-outcome.test.ts` يثبّت التصنيف لأنه أُخطئ مرّتين أثناء كتابته، ولا تراه الأنواع.

**البوابة**: `npm run check` ١٦/١٦، و١٢٢ اختبارًا (كانت ١١٥)، والدخان على ١٢ مسارًا، والصحّة خضراء
بعد النشر، والويبهوك يردّ ٤٠١ لطلب بلا رمز في الإنتاج. **لم تُرسل أي رسالة إلى أي رقم.**

**الخطة**: `docs/designs/massar-platform-implementation.md` عُدّلت بعد أربع مراجعات (CEO، تصميم،
DX، هندسة) بثمانية أصوات مستقلة و١٩ بُعدًا مسجّلًا، كلها سلبية وبلا خلاف بين صوتين. الجدول صار ٣٨
أسبوعًا خامًا و٤٤ مع الاحتياط — أطولَ لا أقصر، لأن التقدير الأول أسقط نظام الهجرات ومنصّة اختبار
لقاعدة البيانات وCI وقرار تخزين الملفات. أُعيدت قاعدة التوقّف (بوابة أ عند الأسبوع ١٦ تقريبًا) بعد
أن سقطت من المسودة الأولى، وسُمّي الجدول المشترك كمنافس، وأُدرجت بيانات العملاء في المستودعين
العامّين في جدول المخاطر.

## 2026-09-03 · [T0] Office hours — Massar scoped as the commercial platform, slice A first

Design record: `docs/designs/massar-commercial-platform.md` (APPROVED). Founder chose the full
product↔sales platform over the rep work-queue wedge that both Claude and Codex recommended; the
doc binds that choice with zero-typing for reps, slice A (work queue + sheet import) shipped first,
evidence gates on every later component, and weekly adoption measurement. Assignment before any
build: audit the last 20 opportunities in the shared sheet for which interaction changed stage and
on which channel, and ask one rep for the last lead lost to no follow-up. Decision id
6e2a8f5e-bb2c-4027-9b73-8347db31240e.

## 2026-09-03 · [T1] Orbit removed — plain repo, knowledge kept under `docs/`

The Orbit agent framework (`.orbit/` runner, roles, checks, CPO rounds, locks, `scripts/orbit-*`,
`ralph_loop.sh`, the 16 role adapters in `.claude/agents/`, and the engine's `.orbit/` mirror +
`.orbit-evidence/`) is gone. What was project knowledge moved, not deleted: this file, the two ADRs
(`docs/decisions/`), the two domain docs (`docs/whatsapp-campaign-engineering.md`,
`docs/massar-design-language.md`), every slice artifact (`docs/artifacts/`), the agent-eval schema +
campaigns-crm RTM (`docs/qa/`), and the design previews (`docs/design-previews/`). The engine's
`eval-agent.mjs` / `eval-judge.sh` / `qa-crm.py` now write there. Older entries below still name
`.orbit/...` paths for things that no longer exist (loop config, baselines, review requests) — read
them as history. CLAUDE.md is rewritten without the gearbox, roster, and loop sections; the safety
rules and the no-send rule are unchanged.

## 2026-08-27 · [T2] «مؤشرات المحادثة» — إشارات بدل النص على ملف العميل · DEPLOYED

Founder: «ردود الذكاء الاصطناعي والتفاصيل لكل عميل عن سير التواصل كثيرة جدًا. نريد إشارات
ورسومًا للعميل، لا نصًا — ما الذي يهتم به، ما مدى جديته، وما توصية المساعد بالخطوة التالية.»

**ما كان**: الملف يجيب عن «كيف تسير المحادثة؟» بسبع كتل نثرية داخل تبويب (خلاصة، إشارات شراء
مقتبسة، اعتراضات، حكم الصفقة، دليله، اقتراح، تبريره) — والتبويب الافتراضي هو «المحادثة»، فالإجابة
كانت خلف نقرة وخلف فقرة.

**ما صار**: بطاقة واحدة **فوق الشبكة** — أي خارج غلاف التبويبات عمدًا، لأن كل ما يوضع داخل
`.crecmain` يُعاد ضمّه إلى تبويب: **الجدية ٠–١٠٠** بشريط عشر كتل وشارة الفئة · **الزخم** ·
**سرعة ردّه** · **آخر كلام منه** · **كم كتب بكلماته** · **مخطط ٢١ يومًا** لمن تكلم ومتى ·
وسطر واحد: اقتراح المساعد للخطوة التالية.

- **الرقم مكسوب، لا مُخمَّن**: `src/signal-domain.ts` طبقة أعمال نقية (١٦ اختبارًا) تحسب من السجل
  وحده — رسالة كتبها بنفسه، طلب تجاري بكلماته، وسم مؤكد، نتيجة سجّلها بشر، سرعة الرد، والصمت.
  **لا نموذج يُنتج أي رقم هنا**؛ المخرج النموذجي الوحيد على البطاقة هو الاقتراح، ومُعلَّم كاقتراح.
  و«لماذا هذه القراءة؟» يفصّل كل بند بدليله — رقمٌ لا يمكن الجدال معه زينة.
- **الانسحاب يُنهي السؤال لا يخصم**: من طلب الإيقاف يقرأ صفرًا ببند واحد، لأن مقياسًا يعطيه ٣٠
  يدعو إلى المتابعة التي يمنعها الإيقاف نفسه.
- **الاهتمام لا يُكرَّر هنا**: يظهر في شريط الحالة أعلاه بعلامة مصدره، وتكراره رابعةً هو ما حذفه
  هذا الملف مرتين من قبل. وحُذف «آخر كلام منه» من سطر الترويسة لأنه صار له بيت معنون تحته.
- **ثلاثة عيوب أمسكها الرسم لا المراجعة**: رسالة واحدة كانت تُرسم عمودًا كامل الارتفاع (أرضية
  للسقف عند ٣)؛ و«٠ دقيقة» كانت أثر تقريب لا زمن ردّ (صارت «أقل من دقيقة»)؛ و«+٠» ليس مساهمة.
- **المحور الزمني يتبع البيت**: أقدم يوم عند حافة البداية واليوم عند النهاية، كما في
  `dailyActivitySvg` — محوران زمنيان متعاكسان على منتج واحد خطأ قراءة ينتظر.

`npm run check` أخضر (١١٥ اختبارًا · تغطية أسطر ٩٤٪) و`npm run smoke` ١٢ مسارًا. تُحقّق على
الإنتاج على رقم حقيقي: «٤٠ · يستحق المتابعة · صامت» ببنود +٢٥ حوار مكتمل، +٢٥ طلب تجاري مقتبسًا،
+١٠ ردّ سريع، −٢٠ صمت ١٥ يومًا.

**الشريحة التالية المسمّاة**: عمود «الاهتمام والجدية» في قائمة العملاء يَعِد بالجدية ولا يعرضها —
هو المكان الذي يُمسح فيه العملاء بالعشرات، وهو الآن استدعاء واحد لنفس الوحدة.

---

## 2026-08-24 · [T3] اعتماد المعايير التقنية — طبقة أعمال حقيقية وبوابة تغطية تمنع النشر · DEPLOYED

Founder, pointing at `Technical Standards.md`: «are you following this standard?»

**الجواب المقيس كان: لا.** صفر اختبارات · لا أداة تغطية · لا `openapi.yaml` · قواعد الأعمال داخل
template literal · وقاعدة بيانات مبنية على أعراف هذا المستودع. `massar-engine/STANDARDS.md` يسجّل
ذلك كله: ما يمتثل، وما لا يمتثل، **ولماذا**، ومخرج كل انحراف — لأن انحرافًا غير معلن لا يمكن تمييزه
عن عيب.

- **§1 طبقة أعمال حقيقية**: `src/opps-domain.ts` — قواعد الفرص كدوال نقية قابلة للاستيراد. كانت
  داخل `opps-crm.ts` كـJS للمتصفح، أي في طبقة العرض، ولهذا لم تُختبر أخطر حسابات المنتج (المال على
  لوحة مبيعات) ولم تكن قابلة للاختبار أصلًا.
- **تعريف واحد، بيئتان**: الخادم يستورد الوحدة، والمتصفح يتلقّى **نفس** المصدر المُصرَّف عبر
  `OPPS_DOMAIN_JS` (`Function.prototype.toString()`). ما يؤكّده الاختبار في Node هو حرفيًا ما تنفّذه
  الصفحة. و`checkDomainClosure()` يحرس القيد الذي لا تحرسه الأنواع: قاعدة تشير إلى ما لم يُشحن معها
  تعمل في Node وتنهار في المتصفح كصفحة بيضاء — تُفحص لا تُفترض، واختبارٌ ينفّذ المصدر المشحون في
  نطاق فارغ ويقارنه بالوحدة.
- **§4 البوابة حيّة ومانعة**: vitest + تغطية v8 عند حدّ المعيار ٦٠٪، و`npm test` صار **أول**
  `npm run check` — فتراجعُ التغطية يوقف النشر كما يوقفه فشل تأكيد. ٧٢ اختبارًا، FIRST + AAA، وبلا
  محاكاة بنية تحتية (§4.3): المستورد يُختبر على ملف xlsx حقيقي يُبنى في الذاكرة، ومنه قالب المنتج
  نفسه. في النطاق: أسطر ٩٢٫٨٥٪ · عبارات ٨٤٫٠٣٪ · دوال ٩٥٫١٢٪ · تفرّعات ٧٠٫٦٦٪.
- **النطاق معلن لا صامت** (D-4): `db.ts` و`index.ts` و`insights.ts` والمحوّلات خارج المقام لأن
  اختبارها وحدويًا يعني محاكاة Postgres وOpenAI — وهو ما يمنعه §4.3؛ وأجزاؤها النقية مُختبَرة على
  أي حال. Testcontainers هي الشريحة التالية المسمّاة، لا محاكاةٌ تُكتب اليوم لتحريك رقم.

**ما وجدته الاختبارات ولم تجده المراجعة**: تعليق يزعم أن الصفر في العربية يأخذ الجمع بينما الشيفرة
تعطيه المفرد (وCLDR مع الشيفرة)؛ وافتراضان مني عن شكل `windowState` و`decideFact` كانا خطأً — ثُبّتا
الآن على العقد الحقيقي.

**وsmoke كان متذبذبًا ولم يعد**: هذا النشر احمرّ على #customer بـ١٦٠ حرفًا، والتشغيل التالي مباشرةً
أعطى ١٨٬٨٨٢ حرفًا ونجح — كان يحكم على آلة ما تزال تُقلع بعد `fly deploy`. صار يستطلع `/health` حتى
تتصل قاعدة البيانات فعلًا قبل أن يحكم. بوابةٌ تفشل لسببٍ لا تختبره بوابةٌ يُعاد تشغيلها بدل أن
تُقرأ — وهكذا يعود صنف الصفحة البيضاء الذي تحرسه.

**مفتوح ومسجَّل**: `openapi.yaml` وقواعد §7 (D-5 — `POST /admin/opps` يعيد ٢٠٠ لا ٢٠١، والأخطاء
ليست RFC 9457)؛ وحكمُ مالك المعيار في §6: هل تنطبق معايير SQL Server على Postgres، وبأي ترجمة
(D-6)؛ وPrettier/ESLint (D-2)؛ والفروع والمراجعة (D-3). engine `774166b`.

---

## 2026-08-24 · [T1] رُفع المشروع إلى GitHub خاص · و«افتح فرصة» الجماعية · DEPLOYED

**الرفع.** مستودعان **خاصّان**، لأن المشروع مستودعان لا واحد:
`github.com/Abdulaziz-almoshen/Massar` (٣٤ commit — النموذج، التصميم، `.orbit`) و
`.../massar-engine` (٢٥٠ commit). عبر **HTTPS** لا SSH: لا مفتاح SSH على هذا الجهاز
(`Permission denied (publickey)`)، فضُبط `gh auth setup-git`.

**فحص الأسرار قبل أول دفعة**: OPENAI_API_KEY · GUPSHUP_API_KEY · GUPSHUP_APP_ID · WEBHOOK_TOKEN ·
ADMIN_TOKEN — **غير موجودة في أي blob في التاريخين** (فُحص ١٨٫٥MB + ٤٠٫٤MB). المتتبَّع الوحيد
`.env.example` وهو فارغ. موجود ومقبول **فقط لأن المستودعين خاصّان**: `NOTIFY_NUMBER` (جوال شخصي)،
وأرقام عملاء حقيقية ونصوص محادثات في `STATE.md` والتجهيزات. **يُعاد الفحص قبل أي إعلان للمستودع.**

**«افتح فرصة» الجماعية** — البند الذي تُرك مفتوحًا. N جهة، خدمة واحدة، بندٌ لكل جهة، **بلا تسعير**
عند «تواصل أولي» — الشكل نفسه الذي تأخذه بنود المساعد التلقائية، فلا تحتاج اللوحة مفردات جديدة.
**قائمة واحدة، فعلان**: أول قصّة وضعت قائمة خدمات ثانية بجانب قائمة الوسم، متطابقة في الشكل، ولا
يستطيع قارئ تمييز أيّهما تحكم أي فعل — لكنها لم تكن سؤالًا ثانيًا أصلًا: «مرشّح لـ فحص الموظفين» هو
قرار مقاربتهم بها، وفتح صفقة عليها هو الخطوة التالية في الجملة نفسها. **ويتخطّى الموجود ويقول ذلك**:
بندٌ ثانٍ على (جهة، خدمة) وهو حيّ أسرع طريق لجعل مجموع خط البيع يكذب. بند **خسارة** لا يمنع — خسارة
خدمة الربع الماضي ليست سببًا لعدم بيعها ثانية. والشريط يفصل: فُتحت / تُخطّيت / تعذّرت.

**تحقّق على الإنتاج** على صفوف «(مثال)» الثلاثة الموجودة أصلًا في الدفتر: التشغيل الأول فتح ٣ بنود
على ٣ أرقام، والتشغيل الثاني المطابق فتح ٠ وأبقى اللوحة على ٣. حُذفت الأربعة بعدها.

**صفّان لم أفسّرهما**: صفّ «DLOOO / الإجازات المرضية / by اللوحة» ظهر مرتين (٥ أمس ١٩:٤٨، و١٦ اليوم
٠٧:٣٥). فحصٌ محدَّد أثبت أن نقرة واحدة تُصدر POST واحدًا بالحمولة الصحيحة، ولا Enter ولا الإلغاء ولا
الانتقال يكتب شيئًا. كلا الوقتين خارج نوافذ اختباراتي — **الأرجح أن المؤسس نفسه جرّب الزر**. حذفتُ
الأول (٥) على افتراض أنه أثر اختبار، وقد يكون عمله؛ تُرك ١٦ له ليقرّر. engine `68cfcca`.
...
- Next: segmentation + retargeting build, from `docs/artifacts/segmentation/BENCHMARK.md`.
...
- 2026-08-13 [T3] **Segmentation UI + agent prompt v2 @c06f71c** (R49 «ابحث… قارن بتسع أدوات ثم ابنِ»,
  ...
  **Scope, stated plainly:** roughly one move of three. An audience can be defined and previewed;
  ...
  and Meta-approved templates. Benchmark: `docs/artifacts/segmentation/BENCHMARK.md` (11 tools).
...
- 2026-08-13 [T3] **Segmentation engine @ea6515c** (R49: «ابحث عنه… قارن بتسع أدوات على الأقل، ثم ابنِ»).
  Benchmark of **11 platforms** first (docs/artifacts/segmentation/BENCHMARK.md): Klaviyo ships the
...
- 2026-08-13 [T3] **UI-quality increment — CPO ACCEPT @1936a22 (9·8·8·8·6, zero musts)**, hardened to
  ...
  **Segmentation research delivered** (R49): 11 platforms benchmarked, `docs/artifacts/segmentation/BENCHMARK.md`.
...
- 2026-08-12 [T4 MISSION] **Overnight rebuild @7db6509→43a5015** (R32 hard rejection + R33
  ...
  docs/artifacts/expert-panel/. Shipped: (1) **win/loss intelligence** — every conversation
  ...
  written: docs/artifacts/night/DEMO-SCRIPT.md. **CPO ROUND-9 ACCEPT @e6f421a —
